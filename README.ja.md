...
> AIエージェントのための継続性レイヤー — AIエージェントは毎セッションでプロジェクトを学び直すべきではありません。
...
**toon-memoryはこの問題を解決します。** AIエージェントのための継続性レイヤー — セッションをまたいでプロジェクトの知識・決定・規約を保存する軽量システムで、各セッションが前回の終わりから始まります。100% ローカルでプライベート、MCP 経由 — クラウドもサーバーも不要。
...
| `memory_pin` | **優先度1-5でエントリをピン留め**：ピン留めされたエントリは、キーワード一致がなくても常にリコール結果の最初に優先度順にソートされて表示される |
| `memory_unpin` | **エントリのピン留めを解除**：優先度フラグを削除 |
| `memory_search` | **フィルター付き統一検索**：`memory_recall`と同じ機能に加え、`category`、`tags`、`from_date`、`to_date`フィルター。タグフィルターはAND論理 — 指定したすべてのタグが一致する必要があります。`sessionBias`で現在のgitブランチのエントリをブースト |
| `memory_tag` | **一括タグ操作**: キーまたはIDで1つ以上のエントリにタグを追加、削除、設定 |

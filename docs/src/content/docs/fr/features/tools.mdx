...
- **Importance explicite** — `memory_remember({ importance })` vous permet de définir `critical` (+0.3), `high` (+0.15), `medium` (0) ou `low` (−0.1) ; les décisions `critical` apparaissent avant les notes `low`, et le niveau apparaît dans les raisons `explain` et la sortie `budget: "deep"`. Vide = automatique (récence + fréquence).

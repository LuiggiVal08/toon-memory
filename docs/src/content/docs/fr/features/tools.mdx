---
title: "40 outils MCP pour la mémoire IA persistante"
description: "40 outils MCP pour la mémoire persistante des agents IA — memory_remember, memory_recall, memory_smart_recall, context_generate et plus encore. Rappel économe en tokens, basé sur le graphe, pour Claude, Cursor, OpenCode."
head:
  - tag: meta
    attrs:
      name: keywords
...
toon-memory fournit 40 outils MCP et 4 ressources MCP pour gérer la mémoire persistante :
...
| `memory_resolve` | **Restaurer une entrée** : restaure une entrée soft-delete (obsolete) en statut actif |
| `memory_suppress` | **Masquer une entrée** : masque une entrée des résultats de rappel sans la supprimer (status=suppressed) |
...
| `memory_supersede` | **Remplacer une entrée** : la marque comme remplacée par une plus récente (`status=obsolete`, liens `superseded_by`/`supersedes`, date `supersededOn`). Les anciennes entrées ne réapparaissent que pour les requêtes `as_of` antérieures à leur remplacement |

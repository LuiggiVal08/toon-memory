...
- **Explizite Wichtigkeit** — `memory_remember({ importance })` setzt `critical` (+0.3), `high` (+0.15), `medium` (0) oder `low` (−0.1); kritische Entscheidungen erscheinen vor niedrigen Notizen; leer = automatisch (Aktualität + Zugriffshäufigkeit).

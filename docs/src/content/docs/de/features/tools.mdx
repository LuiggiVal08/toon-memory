---
title: "40 MCP-Tools für persistenen AI-Speicher"
description: "40 MCP-Tools für persistenen AI-Agent-Speicher — memory_remember, memory_recall, memory_smart_recall, context_generate und mehr. Token-effizienter, graph-bewusster Abruf für Claude, Cursor, OpenCode."
head:
  - tag: meta
    attrs:
      name: keywords
...
toon-memory bietet 40 MCP-Tools und 4 MCP-Ressourcen zur Verwaltung des persistenten Speichers:
...
| `memory_resolve` | **Eintrag wiederherstellen**: stellt einen soft-gelöschten (obsoleten) Eintrag wieder auf aktiv |
| `memory_suppress` | **Eintrag ausblenden**: blendet einen Eintrag aus den Abrufergebnissen aus, ohne ihn zu löschen (status=suppressed) |
...
| `memory_supersede` | **Eintrag ersetzen**: markiert ihn als durch einen neueren ersetzt (`status=obsolete`, `superseded_by`/`supersedes`-Links, `supersededOn`-Datum). Alte Einträge erscheinen nur bei `as_of`-Abfragen vor ihrem Ersatzdatum wieder |

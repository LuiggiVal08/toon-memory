...
description: "40 ferramentas MCP para memória persistente de agentes IA — memory_remember, memory_recall, memory_smart_recall, context_generate e mais. Recall eficiente em tokens e baseado em grafo para Claude, Cursor, OpenCode."
head:
  - tag: meta
    attrs:
      name: keywords
...
O toon-memory fornece 40 ferramentas MCP e 4 recursos MCP para gerenciar memória persistente:
...
| `memory_resolve` | **Restaurar entrada**: restaura uma entrada com soft-delete (obsolete) para o estado ativo |
| `memory_suppress` | **Ocultar entrada**: esconde uma entrada dos resultados de recall sem deletá-la (status=suppressed) |
...
| `memory_supersede` | **Substituir entrada**: marca como substituída por uma mais nova (`status=obsolete`, links `superseded_by`/`supersedes`, data `supersededOn`). Entradas antigas reaparecem apenas em consultas `as_of` anteriores à substituição |

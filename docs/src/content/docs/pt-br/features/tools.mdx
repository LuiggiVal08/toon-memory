...
description: "37 ferramentas MCP para memória persistente de agentes IA — memory_remember, memory_recall, memory_smart_recall, context_generate e mais. Recall eficiente em tokens e baseado em grafo para Claude, Cursor, OpenCode."
head:
  - tag: meta
    attrs:
      name: keywords
...
O toon-memory fornece 37 ferramentas MCP e 4 recursos MCP para gerenciar memória persistente:
...
| `memory_recall` | Busca na memória (use ANTES de ler arquivos, filtra TTL expirado). `mode: "graph"` expande um subgrafo consciente de relacionamentos para maior precisão. Ranking ponderado por qualidade |

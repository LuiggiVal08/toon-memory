...
- **Importância explícita** — `memory_remember({ importance })` permite definir `critical` (+0.3), `high` (+0.15), `medium` (0) ou `low` (−0.1); decisões `critical` aparecem antes de notas `low`, e o nível aparece nas linhas de `explain` e na saída de `budget: "deep"`. Vazio = automático (atualidade + frequência).

...
A linha de motivo `↳` é determinística (relevância %, contagem de acessos, último uso, importância) — sem LLM envolvido. Use `explain: true` quando quiser saber *por que* o agente viu essas entradas.
...
- **`k` adaptativo** — `k = clamp(3..60, round(sqrt(n)))`, onde `n` é a contagem de candidatos. O `k=60` da literatura achata as diferenças de ranking em grafos pequenos, então ele escala com `sqrt(n)`.

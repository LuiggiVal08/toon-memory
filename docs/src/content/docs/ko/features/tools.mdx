...
| 도구 | 설명 |
|------|------|
| `memory_remember` | 결정, 패턴, 버그, 지식 또는 **warning**("하지 마세요" 부정적 메모리, 리콜 부스트 포함) 저장 (선택적 TTL, 자동 태그 추론, `links`, 자동 품질 점수, 중복 병합, 신뢰도) |
...
`↳` 이유 줄은 결정론적입니다 (관련성 %, 접근 횟수, 마지막 사용 시각, 중요도) — LLM이 관여하지 않습니다. 에이전트가 해당 항목을 *왜* 보여주는지 알고 싶을 때 `explain: true`를 사용하세요.

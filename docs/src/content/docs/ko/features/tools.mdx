---
title: "AI 에이전트 영구 메모리를 위한 40개 MCP 도구"
description: "AI 에이전트 영구 메모리를 위한 40개 MCP 도구 — memory_remember, memory_recall, memory_smart_recall, context_generate 등. Claude, Cursor, OpenCode를 위한 토큰 효율적이고 그래프 인식 검색."
head:
  - tag: meta
    attrs:
      name: keywords
...
toon-memory는 영구 메모리를 관리하기 위한 40개의 MCP 도구와 4개의 MCP 리소스를 제공합니다:
...
| `memory_recall` | 메모리 검색 (파일 읽기 전에 사용, 만료된 TTL 필터링). `mode: "graph"`는 더 높은 정확도를 위해 관계 인식 서브그래프를 확장합니다. 순위는 RRF 융합(BM25×3 + 그래프 중심성, 적응형 k)을 사용합니다. `rrf: false`로 기존 선형 점수를 사용할 수 있습니다. `as_of`는 특정 날짜 이후에 superseded된 항목을 다시 포함합니다 |
...
| `memory_resolve` | **항목 복원**: 소프트 삭제(obsolete)된 항목을 활성 상태로 복원합니다 |
| `memory_suppress` | **항목 숨기기**: 삭제하지 않고 리콜 결과에서 숨깁니다 (status=suppressed) |
| `memory_reflect` | **메모리 반성**: 항목을 노후화·품질·과잉 연결별로 결정적으로 순위를 매겨 주의나 정리가 필요한 것을 드러냅니다. LLM 없음 |
| `memory_supersede` | **항목 대체**: 더 새로운 것으로 대체되었음을 표시(`status=obsolete`, `superseded_by`/`supersedes` 링크, `supersededOn` 날짜). 이전 항목은 대체 날짜보다 이른 `as_of` 쿼리에서만 다시 나타납니다 |

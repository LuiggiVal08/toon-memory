...
description: Las 37 herramientas de memoria y contexto y 4 recursos para tu agente de IA
---

## Resumen

toon-memory proporciona 37 herramientas MCP y 4 recursos MCP para gestionar la memoria persistente:
...
| `memory_recall` | Busca en la memoria (úsalo ANTES de leer archivos, filtra entradas con TTL expirado). `mode: "graph"` expande un subgrafo con relaciones para mayor precisión. `sessionBias` potencia entradas de la rama git actual. Ranking con quality scoring |

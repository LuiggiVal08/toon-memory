...
description: Las 40 herramientas de memoria y contexto y 4 recursos para tu agente de IA
---

## Resumen

toon-memory proporciona 40 herramientas MCP y 4 recursos MCP para gestionar la memoria persistente:

## Herramientas

| Herramienta | Descripción |
|------|-------------|
| `memory_remember` | Guarda una decisión, patrón, bug o conocimiento (con TTL opcional, inferencia automática de etiquetas, `links`, quality score automático, merge-dedup, confidence) |
| `memory_recall` | Busca en la memoria (úsalo ANTES de leer archivos, filtra entradas con TTL expirado). `mode: "graph"` expande un subgrafo con relaciones para mayor precisión. El ranking usa fusión RRF (BM25×3 + centralidad del grafo, k adaptativo); pasa `rrf: false` para el score lineal legacy. `sessionBias` potencia entradas de la rama git actual. `as_of` re-incluye entradas superseded después de una fecha puntual |
| `memory_forget` | Elimina una entrada por key o id |
| `memory_stats` | Ver el estado de la memoria (incluyendo estadísticas de TTL, distribución de calidad y memorias frías por debajo de umbrales) |
...
| `memory_consolidate` | De-duplica entradas con contenido idéntico y merge de atributos (determinístico, sin LLM) |
...
| `memory_smart_recall` | Búsqueda unificada: BM25 + grafo + quality + freshness + decay + sesgo de sesión en una sola llamada |
...
| `memory_resolve` | **Restaurar entrada**: restaura una entrada con soft-delete (obsolete) a estado activo |
| `memory_suppress` | **Ocultar entrada**: oculta una entrada de los resultados de recall sin borrarla (status=suppressed) |
...
| `memory_supersede` | **Reemplazar entrada**: márcala como reemplazada por una más nueva (`status=obsolete`, links `superseded_by`/`supersedes`, fecha `supersededOn`). Las antiguas reaparecen solo en consultas `as_of` anteriores a su reemplazo |

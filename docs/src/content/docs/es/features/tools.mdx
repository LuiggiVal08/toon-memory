...
description: Las 34 herramientas de memoria y contexto y 4 recursos para tu agente de IA
---

## Resumen

toon-memory proporciona 34 herramientas MCP y 4 recursos MCP para gestionar la memoria persistente:
...
| `memory_recall` | Busca en la memoria (úsalo ANTES de leer archivos, filtra entradas con TTL expirado). `mode: "graph"` expande un subgrafo con relaciones para mayor precisión. Ranking con quality scoring |
| `memory_forget` | Elimina una entrada por key o id |
| `memory_stats` | Ver el estado de la memoria (incluyendo estadísticas de TTL y distribución de calidad) |
...
| `memory_smart_recall` | Búsqueda unificada: BM25 + grafo + quality + freshness + decay en una sola llamada |
| `memory_pin` | **Fijar entrada**: las entradas fijadas siempre aparecen al inicio de los resultados, incluso sin coincidencia de palabras clave |
| `memory_unpin` | **Desfijar entrada**: elimina la marca de fijado de una entrada |
| `memory_search` | **Búsqueda unificada con filtros**: igual que `memory_recall` más filtros de categoría, etiquetas y rango de fechas |

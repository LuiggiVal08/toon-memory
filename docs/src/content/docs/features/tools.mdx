---
title: "37 MCP Tools for Persistent AI Memory"
description: "37 MCP tools for persistent AI agent memory — memory_remember, memory_recall, memory_smart_recall, context_generate, and more. Token-efficient, graph-aware recall for Claude, Cursor, OpenCode."
head:
  - tag: meta
    attrs:
      name: keywords
...
toon-memory provides 37 MCP tools and 4 MCP resources for managing persistent memory:
...
| `memory_recall` | Search memory (use BEFORE reading files, filters expired TTL). `mode: "graph"` expands a relationship-aware subgraph for higher precision. Quality-weighted ranking. `sessionBias` boosts entries from the current git branch. `pathScope` scopes results to a file path (supports globMatch). `budget`: `"tiny"` (top 3), `"normal"` (top 10), or `"deep"` (top 20) |

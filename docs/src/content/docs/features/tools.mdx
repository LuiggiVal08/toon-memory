...
- **Explicit importance** — `memory_remember({ importance })` lets you set `critical` (+0.3), `high` (+0.15), `medium` (0), or `low` (−0.1); `critical` decisions surface before `low` notes, and the level shows up in `explain` reasons and `budget: "deep"` output. Empty = auto (recency + frequency).

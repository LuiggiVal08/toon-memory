...
| `memory_recall` | Search memory (use BEFORE reading files, filters expired TTL). `mode: "graph"` expands a relationship-aware subgraph for higher precision. Quality-weighted ranking. `sessionBias` boosts entries from the current git branch |

---
title: "27 MCP Tools for Persistent AI Memory"
description: "27 MCP tools for persistent AI agent memory — memory_remember, memory_recall, memory_smart_recall, context_generate, and more. Token-efficient, graph-aware recall for Claude, Cursor, OpenCode."
head:
  - tag: meta
    attrs:
      name: keywords
...
toon-memory provides 30+ MCP tools and 4 MCP resources for managing persistent memory:
...
| `memory_recall` | Search memory (use BEFORE reading files, filters expired TTL). `mode: "graph"` expands a relationship-aware subgraph for higher precision. Quality-weighted ranking |
...
| `memory_smart_recall` | Unified search: BM25 + graph + quality + freshness + decay in one call |

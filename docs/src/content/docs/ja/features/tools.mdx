---
title: "永続AIメモリ向けの40個のMCPツール"
description: "永続AIエージェントメモリ向けの40個のMCPツール — memory_remember、memory_recall、memory_smart_recall、context_generateなど。Claude、Cursor、OpenCode向けのトークン効率の高いグラフ対応リコール。"
head:
  - tag: meta
    attrs:
      name: keywords
...
toon-memoryは永続メモリを管理するための40個のMCPツールと4つのMCPリソースを提供します：

## ツール

| ツール | 説明 |
|------|-------------|
| `memory_remember` | 決定、パターン、バグ、ナレッジを保存（任意のTTL、自動タグ推論、`links`、自動品質スコア、マージ重複排除、信頼度） |
| `memory_recall` | メモリを検索（ファイルを読む前に使用。期限切れTTLをフィルタ）。`mode: "graph"`で関係性を考慮したサブグラフを展開し、より高い精度を実現。ランキングはRRF融合（BM25×3 + グラフ中心性、適応的k）を使用。`rrf: false`で従来の線形スコアに。`as_of`は指定日以降にsupersededされたエントリを再包含します |
| `memory_forget` | キーまたはIDでエントリを削除 |
| `memory_stats` | メモリの状態を表示（TTL統計と品質分布を含む） |
...
| `memory_consolidate` | 同一内容のエントリを重複排除して属性をマージ（決定論的、LLM不使用） |
...
| `memory_smart_recall` | 統合検索: BM25+グラフ+品質+新鮮度+減衰を1回の呼び出しで |
...
| `memory_resolve` | **エントリを復元**: ソフト削除（obsolete）されたエントリをアクティブに復元します |
| `memory_suppress` | **エントリを非表示**: 削除せずにリコール結果から隠します（status=suppressed） |
| `memory_reflect` | **メモリの振り返り**: エントリを陳腐度・品質・過接続で決定的にランク付けし、注意や整理が必要なものを明らかにします。LLM不使用 |
| `memory_supersede` | **エントリを置き換え**: 新しいものに置き換えられたことをマーク（`status=obsolete`、`superseded_by`/`supersedes`リンク、`supersededOn`日付）。古いエントリは置き換え日より前の`as_of`クエリでのみ再表示 |

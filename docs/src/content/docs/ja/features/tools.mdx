---
title: "永続AIメモリ向けの34個のMCPツール"
description: "永続AIエージェントメモリ向けの34個のMCPツール — memory_remember、memory_recall、memory_smart_recall、context_generateなど。Claude、Cursor、OpenCode向けのトークン効率の高いグラフ対応リコール。"
head:
  - tag: meta
    attrs:
      name: keywords
...
toon-memoryは永続メモリを管理するための34個のMCPツールと4つのMCPリソースを提供します：
...
| `memory_recall` | メモリを検索（ファイルを読む前に使用。期限切れTTLをフィルタ）。`mode: "graph"`で関係性を考慮したサブグラフを展開し、より高い精度を実現。品質重み付きランキング |

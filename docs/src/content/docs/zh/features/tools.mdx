---
title: "40 个 MCP 工具实现持久化 AI 记忆"
description: "40 个 MCP 工具用于 AI 代理持久化记忆 — memory_remember、memory_recall、memory_smart_recall、context_generate 等。面向 Claude、Cursor、OpenCode 的 token 高效、图感知召回。"
head:
  - tag: meta
    attrs:
      name: keywords
...
toon-memory 提供 40 个 MCP 工具和 4 个 MCP 资源，用于管理持久化记忆：

## 工具

| 工具 | 描述 |
|------|------|
| `memory_remember` | 保存决策、模式、缺陷或知识（可选 TTL、自动标签推断、`links`、自动质量评分、合并去重、置信度） |
| `memory_recall` | 搜索记忆（应在读取文件之前使用，自动过滤过期 TTL）。`mode: "graph"` 可展开关系感知子图以获得更高精度。排名使用 RRF 融合（BM25×3 + 图中心性，自适应 k）；传入 `rrf: false` 使用旧版线性分数。`as_of` 会重新包含在指定日期之后被 superseded 的条目 |
...
| `memory_consolidate` | 合并去重相同内容的条目并合并属性（确定性，无 LLM） |
...
| `memory_smart_recall` | 统一搜索：BM25 + 图 + 质量 + 新鲜度 + 衰减，一次调用完成 |
...
| `memory_resolve` | **恢复条目**：将软删除（obsolete）的条目恢复为激活状态 |
| `memory_suppress` | **隐藏条目**：从召回结果中隐藏条目而不删除（status=suppressed） |
| `memory_reflect` | **记忆反思**：按过时程度、质量和过度连接确定性排名条目，揭示需要注意或清理的内容。零 LLM |
| `memory_supersede` | **替换条目**：将其标记为已被更新的条目取代（`status=obsolete`、`superseded_by`/`supersedes` 链接、`supersededOn` 日期）。旧条目仅在早于取代日期的 `as_of` 查询中重新出现 |
...
召回是确定性和离线的（无嵌入、无 LLM）。每个候选项条目获得以下综合分数：

- **BM25 相关性** — 对 `id` + `category` + `key` + `content` + `file` + `tags` 的概率性词频评分。
- **图中心性** — 度归一化（0..1）；连接多个条目的枢纽接近 1 分，因此即使没有搜索词也会浮现。
- **重要性** — 新近度 + 访问频率。
- **质量加成** — 质量评分更高的条目（更多标签、链接、详情）获得排序加成。
- **种子加分** — 直接匹配查询的条目获得固定加分。
- **每跳衰减** — 距种子 `d` 跳的节点乘以 `0.5^d`，使远距离上下文低于近距离上下文。

在 `graph` 模式下，召回以关键词匹配为种子，展开自我子图至 `hops` 层，然后按综合分数返回前 `limit`（默认 6）个结果。`memory_smart_recall` 在一次调用中组合所有这些信号。

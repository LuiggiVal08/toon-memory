---
title: "34 个 MCP 工具实现持久化 AI 记忆"
description: "34 个 MCP 工具用于 AI 代理持久化记忆 — memory_remember、memory_recall、memory_smart_recall、context_generate 等。面向 Claude、Cursor、OpenCode 的 token 高效、图感知召回。"
head:
  - tag: meta
    attrs:
      name: keywords
...
toon-memory 提供 34 个 MCP 工具和 4 个 MCP 资源，用于管理持久化记忆：
...
| `memory_recall` | 搜索记忆（应在读取文件之前使用，自动过滤过期 TTL）。`mode: "graph"` 可展开关系感知子图以获得更高精度。质量加权排序 |

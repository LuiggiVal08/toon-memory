...
irm https://raw.githubusercontent.com/LuiggiVal08/toon-memory/main/install.ps1 | iex`,
		...
		tools: {
			...
			cards: [
				...
				{ name: 'memory_smart_recall', title: 'Smart Recall', desc: 'Unified search combining BM25 + graph centrality + quality score + freshness in one call.', group: 'search' },
...
[1] decision/risk-engine-priority
  ...
  tags: risk;spec · edges: ->2, ->3`,
		...
		whatNew: {
			...
			subtitle: 'Pin important entries, unified search with filters, batch tag operations',
			cards: [
				{
					icon: '📌',
					title: 'Pin / Unpin Entries',
					body: 'Pin important entries so they always appear at the top of recall results, even without a keyword match. Unpin removes the flag. Use memory_pin and memory_unpin.',
					stats: ['memory_pin', 'memory_unpin', 'Always on top'],
				},
				{
					icon: '🔎',
					title: 'Unified Search with Filters',
					body: 'Search memory with category, tags, and date range filters. Tag filter uses AND logic — all specified tags must match. Use memory_search.',
					stats: ['memory_search', 'Tag filter', 'Date range'],
				},
				{
					icon: '🏷️',
					title: 'Batch Tag Operations',
					body: 'Add, remove, or set tags on one or more entries by key or id in a single call. Use memory_tag.',
					stats: ['memory_tag', 'Batch ops', 'Add/remove/set'],
				},
				{
					icon: '🧩',
					title: '34 MCP Tools + 4 Resources',
					body: 'Added memory_pin, memory_unpin, memory_search, and memory_tag tools. Total: 34 tools + 4 resources.',
					stats: ['+4 tools', '34 total'],
				},
			],
		},
		cta: {
	...
	es: {
		...
		stats: {
			items: [
				{ number: '31', label: 'Herramientas MCP' },
...
irm https://raw.githubusercontent.com/LuiggiVal08/toon-memory/main/install.ps1 | iex`,
	...
	tools: {
		...
		cards: [
			...
			{ name: 'memory_smart_recall', title: 'Recuperación inteligente', desc: 'Búsqueda unificada combinando BM25 + centralidad + calidad + frescura en una sola llamada.', group: 'search' },
...
[1] decision/risk-engine-priority
  ...
  tags: risk;spec · edges: ->2, ->3`,
	...
	whatNew: {
		title: 'Novedades en v3.4.0',
		subtitle: 'Fija entradas importantes, búsqueda unificada con filtros, operaciones por lotes de etiquetas',
		cards: [
			{
				icon: '📌',
				title: 'Fijar / Desfijar Entradas',
				body: 'Fija entradas importantes para que siempre aparezcan al inicio de los resultados, incluso sin coincidencia de palabras clave. Desfija para eliminar la marca. Usa memory_pin y memory_unpin.',
				stats: ['memory_pin', 'memory_unpin', 'Siempre arriba'],
			},
			{
				icon: '🔎',
				title: 'Búsqueda Unificada con Filtros',
				body: 'Busca en la memoria con filtros de categoría, etiquetas y rango de fechas. El filtro de etiquetas usa lógica AND — todas las etiquetas especificadas deben coincidir. Usa memory_search.',
				stats: ['memory_search', 'Filtro tags', 'Rango fechas'],
			},
			{
				icon: '🏷️',
				title: 'Operaciones por Lotes de Etiquetas',
				body: 'Añade, elimina o establece etiquetas en una o más entradas por key o id en una sola llamada. Usa memory_tag.',
				stats: ['memory_tag', 'Ops por lotes', 'Añadir/eliminar/establecer'],
			},
			{
				icon: '🧩',
				title: '34 Herramientas MCP + 4 Recursos',
				body: 'Se añadieron memory_pin, memory_unpin, memory_search y memory_tag. Total: 34 herramientas + 4 recursos.',
				stats: ['+4 herramientas', '34 total'],
			},
		],
	},
	cta: {
	...
	whatNew: {
		title: 'v3.4.0 新功能',
		subtitle: '固定重要条目、带过滤器的统一搜索、批量标签操作',
		cards: [
			{
				icon: '📌',
				title: '固定 / 取消固定条目',
				body: '固定重要条目，使其始终出现在召回结果顶部，即使没有关键字匹配。取消固定可移除标记。使用 memory_pin 和 memory_unpin。',
				stats: ['memory_pin', 'memory_unpin', '始终置顶'],
			},
			{
				icon: '🔎',
				title: '带过滤器的统一搜索',
				body: '使用类别、标签和日期范围过滤器搜索记忆。标签过滤使用 AND 逻辑 — 所有指定标签必须匹配。使用 memory_search。',
				stats: ['memory_search', '标签过滤', '日期范围'],
			},
			{
				icon: '🏷️',
				title: '批量标签操作',
				body: '在单次调用中按 key 或 id 对一个或多个条目添加、删除或设置标签。使用 memory_tag。',
				stats: ['memory_tag', '批量操作', '添加/删除/设置'],
			},
			{
				icon: '🧩',
				title: '34 个 MCP 工具 + 4 个资源',
				body: '新增 memory_pin、memory_unpin、memory_search 和 memory_tag。总计：34 个工具 + 4 个资源。',
				stats: ['+4 工具', '34 总计'],
			},
		],
	},
	...
	whatNew: {
		title: 'v3.4.0 新機能',
		subtitle: 'エントリのピン留め、フィルター付き統合検索、一括タグ操作',
		cards: [
			{
				icon: '📌',
				title: 'ピン留め / 解除',
				body: '重要なエントリをピン留めして、キーワード一致がなくても常にリコール結果の先頭に表示。解除でフラグを削除。memory_pin / memory_unpin を使用。',
				stats: ['memory_pin', 'memory_unpin', '常に先頭'],
			},
			{
				icon: '🔎',
				title: 'フィルター付き統合検索',
				body: 'カテゴリ、タグ、日付範囲フィルタでメモリを検索。タグフィルタは AND 論理 — 指定した全タグが一致する必要があります。memory_search を使用。',
				stats: ['memory_search', 'タグフィルタ', '日付範囲'],
			},
			{
				icon: '🏷️',
				title: '一括タグ操作',
				body: '1 回の呼び出しで key または id で 1 つ以上のエントリにタグを追加、削除、設定。memory_tag を使用。',
				stats: ['memory_tag', '一括操作', '追加/削除/設定'],
			},
			{
				icon: '🧩',
				title: '34 MCPツール + 4 リソース',
				body: 'memory_pin、memory_unpin、memory_search、memory_tag を追加。合計：34ツール + 4リソース。',
				stats: ['+4 ツール', '34 合計'],
			},
		],
	},
	...
	whatNew: {
		title: 'v3.4.0 새로운 기능',
		subtitle: '항목 고정, 필터가 있는 통합 검색, 일괄 태그 작업',
		cards: [
			{
				icon: '📌',
				title: '항목 고정 / 고정 해제',
				body: '중요한 항목을 고정하여 키워드 일치가 없어도 항상 리콜 결과 상단에 표시. 고정 해제로 플래그 제거. memory_pin / memory_unpin 사용.',
				stats: ['memory_pin', 'memory_unpin', '항상 상단'],
			},
			{
				icon: '🔎',
				title: '필터가 있는 통합 검색',
				body: '카테고리, 태그, 날짜 범위 필터로 메모리 검색. 태그 필터는 AND 로직 사용 — 지정된 모든 태그가 일치해야 함. memory_search 사용.',
				stats: ['memory_search', '태그 필터', '날짜 범위'],
			},
			{
				icon: '🏷️',
				title: '일괄 태그 작업',
				body: '한 번의 호출로 key 또는 id로 하나 이상의 항목에 태그 추가, 제거 또는 설정. memory_tag 사용.',
				stats: ['memory_tag', '일괄 작업', '추가/제거/설정'],
			},
			{
				icon: '🧩',
				title: '34개 MCP 도구 + 4개 리소스',
				body: 'memory_pin, memory_unpin, memory_search, memory_tag 추가. 총계: 34개 도구 + 4개 리소스.',
				stats: ['+4 도구', '34 총계'],
			},
		],
	},
...
[1] decision/risk-engine-priority
  ...
  tags: risk;spec · edges: ->2, ->3`,
	...
	whatNew: {
		title: 'Novidades na v3.4.0',
		subtitle: 'Fixe entradas importantes, busca unificada com filtros, operações em lote de tags',
		cards: [
			{
				icon: '📌',
				title: 'Fixar / Desfixar Entradas',
				body: 'Fixe entradas importantes para que apareçam sempre no topo dos resultados, mesmo sem correspondência de palavras-chave. Desfixar remove a marca. Use memory_pin e memory_unpin.',
				stats: ['memory_pin', 'memory_unpin', 'Sempre no topo'],
			},
			{
				icon: '🔎',
				title: 'Busca Unificada com Filtros',
				body: 'Pesquise na memória com filtros de categoria, tags e intervalo de datas. Filtro de tags usa lógica AND — todas as tags especificadas devem corresponder. Use memory_search.',
				stats: ['memory_search', 'Filtro tags', 'Intervalo datas'],
			},
			{
				icon: '🏷️',
				title: 'Operações em Lote de Tags',
				body: 'Adicione, remova ou defina tags em uma ou mais entradas por key ou id em uma única chamada. Use memory_tag.',
				stats: ['memory_tag', 'Ops em lote', 'Adicionar/remover/definir'],
			},
			{
				icon: '🧩',
				title: '34 Ferramentas MCP + 4 Recursos',
				body: 'Adicionados memory_pin, memory_unpin, memory_search e memory_tag. Total: 34 ferramentas + 4 recursos.',
				stats: ['+4 ferramentas', '34 total'],
			},
		],
	},
	cta: {
...
whatNew: {
	title: 'Neu in v3.4.0',
	subtitle: 'Einträge anheften, einheitliche Suche mit Filtern, Batch-Tag-Operationen',
	cards: [
		{
			icon: '📌',
			title: 'Eintrag anheften / lösen',
			body: 'Heftet wichtige Einträge an, sodass sie immer oben in den Ergebnissen erscheinen, auch ohne Keyword-Übereinstimmung. Lösen entfernt die Markierung. Verwende memory_pin und memory_unpin.',
			stats: ['memory_pin', 'memory_unpin', 'Immer oben'],
		},
		{
			icon: '🔎',
			title: 'Einheitliche Suche mit Filtern',
			body: 'Durchsuche den Speicher mit Kategorie-, Tag- und Datumsbereichsfiltern. Der Tag-Filter verwendet UND-Logik — alle angegebenen Tags müssen übereinstimmen. Verwende memory_search.',
			stats: ['memory_search', 'Tag-Filter', 'Datumsbereich'],
		},
		{
			icon: '🏷️',
			title: 'Batch-Tag-Operationen',
			body: 'Füge Tags hinzu, entferne oder setze sie bei einem oder mehreren Einträgen per Key oder ID in einem Aufruf. Verwende memory_tag.',
			stats: ['memory_tag', 'Batch-Ops', 'Hinzufügen/entfernen/setzen'],
		},
		{
			icon: '🧩',
			title: '34 MCP-Tools + 4 Ressourcen',
			body: 'memory_pin, memory_unpin, memory_search und memory_tag hinzugefügt. Gesamt: 34 Tools + 4 Ressourcen.',
			stats: ['+4 Tools', '34 gesamt'],
		},
	],
},
cta: {
...
whatNew: {
	title: 'Nouveautés dans v3.4.0',
	subtitle: 'Épinglez des entrées importantes, recherche unifiée avec filtres, opérations par lots sur les tags',
	cards: [
		{
			icon: '📌',
			title: 'Épingler / Détacher des entrées',
			body: 'Épinglez des entrées importantes pour qu\'elles apparaissent toujours en haut des résultats, même sans correspondance de mot-clé. Détacher supprime le marqueur. Utilisez memory_pin et memory_unpin.',
			stats: ['memory_pin', 'memory_unpin', 'Toujours en haut'],
		},
		{
			icon: '🔎',
			title: 'Recherche unifiée avec filtres',
			body: 'Recherchez dans la mémoire avec des filtres par catégorie, tags et plage de dates. Le filtre de tags utilise la logique ET — tous les tags spécifiés doivent correspondre. Utilisez memory_search.',
			stats: ['memory_search', 'Filtre tags', 'Plage dates'],
		},
		{
			icon: '🏷️',
			title: 'Opérations par lots sur les tags',
			body: 'Ajoutez, supprimez ou définissez des tags sur une ou plusieurs entrées par key ou id en un seul appel. Utilisez memory_tag.',
			stats: ['memory_tag', 'Ops par lots', 'Ajouter/supprimer/définir'],
		},
		{
			icon: '🧩',
			title: '34 Outils MCP + 4 Ressources',
			body: 'Ajout de memory_pin, memory_unpin, memory_search et memory_tag. Total : 34 outils + 4 ressources.',
			stats: ['+4 outils', '34 total'],
		},
	],
},
cta: {

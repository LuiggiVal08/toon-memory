...
[1] decision/risk-engine-priority
  ...
  tags: risk;spec · edges: ->2, ->3`,
		...
		whatNew: {
			...
			subtitle: 'Smarter auto-loading, new tools, and quality improvements across the board',
			cards: [
				{
					icon: '⚡',
					title: 'Intelligent Auto-Loading',
					body: 'OpenCode plugin auto-recalls relevant memory by file path on every tool execution — no dump-all, no wasted context. Injects only what matters via setContext().',
					stats: ['~90% fewer tokens', 'File-path recall'],
				},
				{
					icon: '📉',
					title: 'Staleness Decay',
					body: 'Quality scoring now penalizes entries not accessed in 30+ days (up to -0.2). The freshest, most relevant knowledge surfaces first in recall results.',
					stats: ['30-day ramp', 'Auto-quality boost'],
				},
				{
					icon: '🔀',
					title: 'Jaccard Merge & Graph Path',
					body: 'memory_merge_similar finds and merges near-duplicates deterministically (Jaccard >50%). memory_graph_path finds BFS shortest-path chains between any two entries.',
					stats: ['No LLM needed', 'BFS pathfinding'],
				},
				{
					icon: '🤖',
					title: '15 Agents Configured',
					body: 'Added Continue, Zed, Antigravity, KiloCode, and OpenClaw with correct paths and formats. Each agent gets proper MCP config, hooks, and capture hooks.',
					stats: ['5 new agents', 'Corrected paths'],
				},
			],
		},
		cta: {

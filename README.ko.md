...
- **MCP 도구 21개** — Model Context Protocol을 통한 전체 메모리 관리. `memory_smart_recall` (통합 리콜), `memory_sessions` 다세션 협업, `context_*` 도구를 통한 원 호출 컨텍스트 생성(브리핑, 차이점, 집중, 건강 감사, 내보내기) 포함
...
│   ├── mcp/
│   │   └── server.ts            # MCP 서버 (21개 도구 + 3개 리소스)
│   ├── lib/

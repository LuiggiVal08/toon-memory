...
> MCP-Speicherserver für KI-Coding-Agenten — speichere Entscheidungen, Muster und Fehler zwischen Sitzungen.
...
**toon-memory löst dieses Problem.** Es gibt deinem KI-Agenten einen dauerhaften Speicher, der Neustarts überlebt, sodass er tatsächlich aus deinem Projekt lernt.
...
- **21 MCP-Werkzeuge** — Vollständige Speicherverwaltung über das Model Context Protocol, inklusive `memory_smart_recall` (einheitlicher Recall), `memory_sessions` zur Mehrzellen-Koordination und `context_*`-Werkzeuge zur Kontextgenerierung mit einem Aufruf (Briefing, Diff, Focus, Gesundheitsprüfung, Export)
...
| **Antigravity** | `.gemini/config/mcp_config.json` + `.gemini/config/hooks.json` | hooks.json | PreInvocation + PostToolUse + Stop (kein SessionStart-Event) | ✅ |
| **Aider** | — | — | — | 📝 Anleitung |
...
| `memory_recall` | Durchsuche den Speicher (VOR dem Lesen von Dateien verwenden, abgelaufene TTL wird gefiltert). `mode: "graph"` erweitert einen beziehungsbewussten Subgraphen für höhere Präzision. `compact: true` gibt ein token-effizientes, nummerisch indexiertes Format zurück. Qualitätsbewertetes Ranking |
| `memory_smart_recall` | **Einheitlicher Recall**: BM25 + Graph + Decay + Qualität in einem Aufruf. Am ANFANG jeder Aufgabe verwenden. Gibt kompaktes, token-effizientes Ergebnis zurück |
| `memory_forget` | Entferne einen Eintrag anhand des Keys oder der ID |
| `memory_stats` | Zeige den Speicherzustand (inklusive TTL-Statistiken und Qualitätsverteilung) |
...
│   ├── mcp/
│   │   └── server.ts            # MCP-Server (21 Werkzeuge + 3 Ressourcen)
│   ├── lib/
